...
class MeshWebRTCService extends EventEmitter {
  ...
  async connect(mediaServerUrl = import.meta.env.VITE_MEDIA_SERVER_URL) {
  ...
  setupSocketListeners() {
    ...
    this.socket.on("newProducer", ({ producerId, peerId, kind }) => {
      console.log(
        `New producer: ${producerId} from peer: ${peerId}, kind: ${kind}`
      );
  ...
  async createPeerConnection(peerId, isInitiatorFlag = false) {
    ...
    console.log(
      `Creating peer connection for ${peerId}, initiator: ${isInitiatorFlag}`
    );
  ...
  async createReceiveTransport() {
    console.log(
      "Create receive transport - handled by peer connections in mesh"
    );
    return Promise.resolve();
  }
