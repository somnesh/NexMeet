...
class MediasoupService extends EventEmitter {
    ...
    connect(mediaServerUrl = import.meta.env.VITE_MEDIASOUP_URL) {
    ...
    async createSendTransport() {
        return new Promise((resolve, reject) => {
            this.socket.emit('createWebRtcTransport', { direction: 'send' }, async (response) => {
                if (response.error) {
                    reject(new Error(response.error));
                    return;
                }
    ...
    async createReceiveTransport() {
        return new Promise((resolve, reject) => {
            this.socket.emit('createWebRtcTransport', { direction: 'receive' }, async (response) => {
                if (response.error) {
                    reject(new Error(response.error));
                    return;
                }
        ...
        return new Promise((resolve, reject) => {
            this.socket.emit('consume', {
                ...
                try {
                    ...
                    this.socket.emit('resumeConsumer', { consumerId: consumer.id }, (response) => {
                            if (response.error) {
